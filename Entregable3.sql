...
#------------------------------------------------------------------------
DELIMITER //
CREATE PROCEDURE actualizar_superficie_concello (IN nombre_concello VARCHAR(30), IN nueva_superficie FLOAT)
BEGIN
   UPDATE Datos SET superficie = nueva_superficie WHERE concello = nombre_concello;
END //
DELIMITER ;
...
#------------------------------------------------------------------------
drop procedure if exists borra_datos_por_superficie;
DELIMITER //
...
## call borra_datos_por_superficie(0,50);
## select * from Datos where Superficie >= 50 AND Superficie <= 100 order by superficie; 
## select * from Datos;
