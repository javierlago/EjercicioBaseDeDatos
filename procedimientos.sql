#------------------------------------------------------------------------
#	CREAMOS LA BASE DE DATOS DatosGalicia y BD_COPIA
#------------------------------------------------------------------------
	DROP DATABASE IF EXISTS DatosGalicia;
	CREATE DATABASE DatosGalicia;
	USE DatosGalicia;
	
    drop database if exists  bd_copia;
    create database  bd_copia;
CREATE TABLE Datos (
...
CREATE TABLE bd_copia.datos_copia LIKE Datos;
...
#------------------------------------------------------------------------
	DELIMITER //
        ...
        FOR EACH ROW
        BEGIN
			INSERT INTO datos_copia				
						VALUES (new.idDatos, new.Provincia, new.Comarca, new.Concello, new.Superficie, new.Mujeres, New.Hombres);
			INSERT INTO bd_copia.datos_copia				
						VALUES (new.idDatos, new.Provincia, new.Comarca, new.Concello, new.Superficie, new.Mujeres, New.Hombres);
...
#------------------------------------------------------------------------
	DELIMITER //
        ...
        BEGIN
				update datos_copia
						set Provincia=new.Provicia,Caomarca=new.Comarca,Concello=new.Concello;
        END
    // DELIMITER ;
#------------------------------------------------------------------------
	DELIMITER //
		CREATE 
    TRIGGER  BorrarDatos
 BEFORE DELETE ON Datos FOR EACH ROW 
    BEGIN END
// DELIMITER ;
#------------------------------------------------------------------------
...
#select * from Datos where idDatos = 2;
#select * from BD_copia.Datos_copia where idDatos = 2;
...
#------------------------------------------------------------------------
	DELIMITER //
        ...
        CREATE FUNCTION SuperficieComarcaGalicia( ComarcaBuscar varchar( 30 ) ) 
            ...
            BEGIN
		# Declaramos variable
			declare superficies float;
            
		# Asignamos  valor a variable
			set superficie =(select sum(superficie) from Datos where Comarca=comarcaBuscar);
